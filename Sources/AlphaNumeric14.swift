public class AlphaNumeric14 : HT16K33 {
    ...
    func setRawBitmask(pos: Int, bitmask: UInt16, write: Bool = true) {
        ...
        self.buffer[pos * 2] = UInt8(bitmask & 0xFF)
        self.buffer[(pos * 2) + 1] = UInt8((bitmask >> 8) & 0xFF)
        if write {
            self.write()
        }
    }
    
    func setDecimal(pos: Int, on: Bool) {
        if on {
            self.buffer[(pos * 2) + 1] = self.buffer[(pos * 2) + 1] | (1 << 6)
        } else {
            self.buffer[(pos * 2) + 1] = self.buffer[(pos * 2) + 1] & ~(1 << 6)
        }
    }
